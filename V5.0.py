        ...
        with st.expander('Entrada de Variáveis'):
            with st.form('form_var_simu'):
                ...
                if nivel in ['Fábrica', 'Gestão']:
                    ...
                    st.subheader('Cozedores')
                    col1, col2, col3, col4 = st.columns(4)
                    Brix_Said_CozA = col1.number_input('Brix de saída do Cozedor Massa A (º)', min_value=0.0, value=90.0)
                    Brix_Said_CozB = col2.number_input('Brix de saída do Cozedor Massa B (º)', min_value=0.0, value=89.0)
                    Purez_Said_CozA = col3.number_input('Pureza do Caldo do Cozedor Massa A (%)', min_value=0.0, value=88.0) / 100
                    Purez_Said_CozB = col4.number_input('Pureza do Caldo do Cozedor Massa B (%)', min_value=0.0, value=57.0) / 100
                    st.subheader('Cristalizador')
                    Brix_Said_Crist = st.number_input('Brix na Saída do Cristalizador (º)', min_value=0.0, value=97.0)
                    st.subheader('Centrífuga')
                    col1, col2, col3, col4 = st.columns(4)
                    Brix_Mel_A = col1.number_input('Brix do Mel A (%)', min_value=0.0, value=80.94)
                    Purez_Mel_A = col2.number_input('Pureza do Mel A (%)', min_value=0.0, value=75.41) / 100
                    Brix_Mel_Final = col3.number_input('Brix do Mel Final (%)',min_value=0.0, value=67.92)
                    Purez_Mel_Final = col4.number_input('Pureza do Mel Final (%)', min_value=0.0, value=58.26) / 100
                    st.divider()
